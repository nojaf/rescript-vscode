...
exports.getMonorepoRootFromBinaryPath = exports.findBinary = void 0;
const fs = require("fs");
...
const platformDirArm64 = process.arch === "arm64" ? process.platform + process.arch : null;
const platformDirGeneric = process.platform;
const normalizePath = (filePath) => {
    return filePath != null ? path.normalize(filePath) : null;
};
const findFilePathFromProjectRoot = (directory, filePartialPath) => {
    if (directory == null) {
        return null;
    }
    ...
    return findFilePathFromProjectRoot(normalizePath(parentDirStr), filePartialPath);
};
const findBinary = async ({ projectRootPath, binary, platformPath, }) => {
    ...
    if (projectRootPath !== null) {
        try {
            const compilerInfo = path.resolve(projectRootPath, compilerInfoPartialPath);
    ...
    const rescriptDir = findFilePathFromProjectRoot(projectRootPath, path.join("node_modules", "rescript"));
    if (rescriptDir == null) {
        return null;
    }
    ...
    if (binary === "rescript") {
        ...
        const targetPackagePath = path.join(fs.realpathSync(rescriptDir), "..", `@rescript/${target}/bin.js`);
        const { binPaths } = await Promise.resolve(`${url.fileURLToPath(targetPackagePath)}`).then(s => require(s));
        if (binary === "bsc.exe") {
 ...
 */
const getMonorepoRootFromBinaryPath = (binaryPath) => {
    if (binaryPath == null) {
        return null;
    }
...
exports.getMonorepoRootFromBinaryPath = getMonorepoRootFromBinaryPath;
